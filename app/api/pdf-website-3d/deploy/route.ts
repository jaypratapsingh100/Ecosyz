...
export async function POST(req: NextRequest) {
  try {
    ...
    const title = (body.title as string | undefined)?.trim() || '3D Portfolio';
    ...
    const slug = title
      ...
      .replace(/-+/g, '-')
      .toLowerCase()
      .slice(0, 30) || '3d-portfolio';
    const projectName = `pdf-website-3d-${slug}-${Date.now().toString(36)}`;
    ...
    console.error('[pdf-website-3d/deploy] Error:', error);
