 ...
 * Used by the chat UI to show provider/model selector (Groq, OpenRouter, Anthropic Claude).
 */
export async function GET() {
  ...
  return NextResponse.json({
    ...
    models: {
      ...
      anthropic: ANTHROPIC_APP_BUILDER_MODELS.map((m) => ({ id: m.id, label: m.label })),
    },
  });
}
